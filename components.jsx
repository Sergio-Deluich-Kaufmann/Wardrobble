...
// ── FAB ──────────────────────────────────────────────────────────
const FAB = ({ onClick, children, size = 56, style: extraStyle = {} }) => (
  <button onClick={onClick} style={{
...
// ── Add Item Sheet ───────────────────────────────────────────────
const CATS      = ['Tops','Bottoms','Outerwear','Shoes','Accessory'];
const OCCASIONS = [
...
// Inline item preview card
const ItemPreview = ({ cat, color, name, imgSrc }) => {
  const bg = COLOR_HEX[color] || '#E8E4E4';
  return (
    <div style={{
      ...
      <div style={{ height: 110, position: 'relative', overflow: 'hidden' }}>
        {imgSrc ? (
          ...
          <ClothingPlaceholder
            ...
            style={{ width: '100%', height: '100%', transition: 'all 0.22s ease' }}
          />
        )}
      </div>
      {/* Info */}
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ overflowY: 'auto', padding: '0 24px 40px', flex: 1 }}>
          ...
          <div style={{ display:'flex', gap: 6, flexWrap:'wrap', marginBottom: 20 }}>
            {CATS.map(c => (
              <Pill key={c} active={cat === c} onClick={() => setCat(c)} small>{c}</Pill>
            ))}
          </div>
            ...
            {COLORS.map(col => {
              ...
              return (
                <button key={col} onClick={() => setColor(col)} style={{
                  ...
                  <span style={{ ...T.caption, fontWeight: 600, color: isActive ? C.primary : C.secondary }}>
                    {col}
                  </span>
                </button>
              );
...
Object.assign(window, {
  ...
  TopAppBar, BottomNav, Pill, FAB, Tag, SectionHeader, AddItemSheet,
  CATS, COLORS, OCCASIONS,
});
