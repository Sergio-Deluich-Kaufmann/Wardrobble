...
import { PlusIcon, TrashIcon, ClothingPlaceholder } from '../icons';
import { TopAppBar, SectionHeader, Pill, FAB, Tag, AddItemSheet } from '../components';
    ...
    >
      <div style={{ height: 118, position: 'relative', overflow: 'hidden' }}>
        {item.imgSrc ? (
          ...
          <ClothingPlaceholder category={item.category} color={item.color} width={120} height={118} style={{ width: '100%', height: '100%' }} />
        )}
        <div style={{
      ...
      <div style={{ padding: '8px 9px 10px' }}>
        <div style={{ ...T.h4, fontSize: 11, color: C.dark, marginBottom: 5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {item.name}
        </div>
      ...
      <div style={{ position: 'absolute', top: 64, bottom: 80, left: 0, right: 0, overflowY: 'auto', padding: '24px 16px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          ...
          <button onClick={() => setShowAdd(true)} style={{
            ...
            <div style={{ width: 38, height: 38, borderRadius: '50%', background: C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
