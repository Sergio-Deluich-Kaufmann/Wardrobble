...
import { SearchIcon, CloseIcon, TrashIcon, PlusIcon, ClothingPlaceholder } from '../icons';
import { TopAppBar, SectionHeader } from '../components';
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? '0 8px 28px rgba(20,27,43,0.1)' : '0 2px 12px rgba(20,27,43,0.06)',
    ...
    >
      <div style={{ display: 'flex', height: 96, overflow: 'hidden' }}>
        {outfit.items.slice(0, 4).map((item, i) => (
          <div key={i} style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
            <ClothingPlaceholder category={item.category} color={item.color} width={100} height={96} style={{ width: '100%', height: '100%' }} />
      ...
      <div style={{ padding: '14px 16px 16px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }}>
          ...
          <button onClick={() => onDelete(outfit.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, borderRadius: 8 }}>
            <TrashIcon size={14} />
          </button>
        </div>
      </div>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid rgba(220,192,191,0.2)', padding: '12px 16px 16px' }}>
...
const CreateOutfitSheet = ({ wardrobe, onClose, onSave, defaultName }) => {
  ...
  const handleSave = () => {
    if (!canSave) return;
    const items = chosen.map(i => ({ category: i.category, color: i.color, name: i.name }));
    const d = new Date();
            ...
            return (
              <div key={slot.key} style={{ marginBottom: 18 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  {picked && <span style={{ ...T.caption, color: C.dark, fontWeight: 600 }}>{picked.name}</span>}
                </div>
                {candidates.length === 0 ? (
                  <div style={{ padding: '14px', borderRadius: 12, background: C.cardBg, ...T.caption, color: C.muted, textAlign: 'center' }}>
                    No {slot.label.toLowerCase()} in your wardrobe yet.
                  </div>
                ) : (
    ...
    <div style={{ position: 'absolute', inset: 0, background: C.bg }}>
      ...
      <div style={{ position: 'absolute', top: 64, bottom: 80, left: 0, right: 0, overflowY: 'auto', padding: '24px 16px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 22, alignItems: 'center' }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: C.cardBg, borderRadius: 999, padding: '10px 14px', border: '1px solid rgba(220,192,191,0.15)' }}>
            <SearchIcon size={14} />
            <input
              value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search archive…"
              style={{ border: 'none', background: 'transparent', outline: 'none', ...T.body, color: C.dark, width: '100%' }}
            />
            {search && (
        ...
        <button
          ...
          style={{
            ...
            background: C.white, border: '1px solid rgba(20,27,43,0.18)',
